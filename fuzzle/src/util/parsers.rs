...
pub fn sticker_id_literal(input: &str) -> IResult<&str, &str> {
    let (input, result) = recognize(many1(alt((alphanumeric1, tag("-"), tag("_"))))).parse(input)?;
...
pub fn set_name_literal(input: &str) -> IResult<&str, &str> {
    recognize( tuple((satisfy(|c| c.is_ascii_alphabetic()), opt(tag("_")), separated_list1(tag("_"), alphanumeric1)))).parse(input)
}

pub fn parse_emoji(input: &str) -> IResult<&str, Emoji> {
    let (emoji, input) = parse_first_emoji(input);
    emoji.map_or_else(|| fail().parse(input), |emoji| success(emoji).parse(input))
}
