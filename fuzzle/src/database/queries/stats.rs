...
where user_id is not null and (tag.id = (select t.id from tag t where t.linked_user_id = sticker_set.created_by_user_id limit 1) or tag.id is null)
group by sticker_set.created_by_user_id 
