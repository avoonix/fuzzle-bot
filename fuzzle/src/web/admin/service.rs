...
use crate::{
    database::{BanReason, BannedSticker, Sticker, StickerSet}, services::Services, sticker::{
        create_historgram_image, create_sticker_thumbnail, fetch_sticker_file, generate_merge_image,
    }, util::Required, web::shared::{AppState, thumbnail_cache_control_header}
};
use web::Data;
...
async fn get_similar_stickers(
    ...
    let (matches, _) = data.services.similarity.find_similar_stickers(sticker_id, crate::inline::SimilarityAspect::Embedding, 100, 0).await?;
    let result = data.services.similarity.matches_to_stickers(matches).await?;
    Ok(actix_web::web::Json(
        result.into_iter()
            .map(|(s, similarity)| StickerWithSimilarityPub {sticker: StickerPub::from(s), similarity})
            .collect_vec(),
    ))
}
...
async fn ban_sticker(
    ...
    data.services.import.ban_sticker(&sticker_id, body.clip_max_match_distance, BanReason::Manual).await?;
    Ok(HttpResponse::Ok().finish())
}
...
async fn approve_set(
    ...
    data.database.set_sticker_set_pending(&set_id, false).await?;
    Ok(HttpResponse::Ok().finish())
}

#[actix_web::post("/api/scan-all-stickers-for-bans")]
#[tracing::instrument(skip(data))]
async fn scan_all_stickers_for_bans(
    data: Data<AppState>,
) -> actix_web::Result<impl Responder> {
    tokio::task::spawn(async move {
        let stickers = data.vector_db.scroll_stickers().await.unwrap();
        dbg!(stickers.len());
        dbg!(stickers.first());
        for (clip_vec, histogram_vec, file_id) in stickers {
            let sticker = data.database.get_some_sticker_by_file_id(&file_id).await.unwrap();
            if let Some(sticker)  = sticker {
                data.services.import.possibly_auto_ban_sticker(clip_vec, &sticker.id).await.unwrap();
            }
        }
...
async fn get_banned_sticker_thumbnail(
    ...
    let sticker = data.database.get_banned_sticker(&sticker_id).await?.unwrap();
    let file_id = sticker
        .thumbnail_file_id
        .required()?;
