...
static TOKENIZER: Lazy<Mutex<SimpleTokenizer>> = Lazy::new(|| {
    ...
    gz.read_to_string(&mut bpe_vocab)
        .expect("hardcoded vocabulary to be read");
    Mutex::new(SimpleTokenizer::new(&bpe_vocab))
});

/// input is a slice of all possible tags (they are all encoded in bulk)
pub fn tokenize(input: &[String]) -> Result<Tensor, EmbeddingError> {
    let mut tokenizer = TOKENIZER.lock().unwrap_or_else(PoisonError::into_inner);
    let sot_token = tokenizer.encoder["<|startoftext|>"];
    let eot_token = tokenizer.encoder["<|endoftext|>"];

    let all_tokens: Result<Vec<Vec<usize>>, EmbeddingError> = input
        .iter()
        .map(|text| {
            Ok(vec![sot_token]
                .into_iter()
    ...
    let result: Tensor = tract_ndarray::Array2::from_shape_fn(
        ...
        |(input_idx, token_idx)| {
            let tokens = all_tokens.get(input_idx).cloned().unwrap_or_default();
...
fn bytes_to_unicode() -> IndexMap<u32, char> {
    ...
    let cs = cs.into_iter().map(|c| char::from_u32(c).unwrap_or('�'));
...
fn whitespace_clean(text: &str) -> String {
    let cleaned_text = regex::Regex::new(r"\s+")
        .expect("hardcoded regex to parse")
        .replace_all(text, " ")
        .to_string();
...
impl SimpleTokenizer {
    ...
    fn bpe(&mut self, token: &str) -> Result<String, EmbeddingError> {
        if let Some(cached) = self.cache.get(token) {
            return Ok(cached.clone());
        }
        let mut word = token.chars().map(|c| c.to_string()).collect_vec();
        let last = word.pop().ok_or(EmbeddingError::Other(anyhow::anyhow!("zero char token")))?;
    ...
    fn encode(&mut self, text: &str) -> Result<Vec<usize>, EmbeddingError> {
    ...
    fn decode(&self, tokens: Vec<usize>) -> String {
        let text = tokens
            ...
            .join("");
        text.chars()
            .map(|c| char::from_u32(self.byte_decoder[&c]).unwrap_or('�'))
            .collect::<String>()
            .replace("</w>", " ")
    }
}
...
mod tests {
    ...
    fn test_internal() -> anyhow::Result<()> {
        ...
        let mut tokenizer = TOKENIZER.lock().unwrap_or_else(PoisonError::into_inner);
        assert_eq!(tokenizer.bpe("dog")?, "dog</w>");
        assert_eq!(tokenizer.bpe(".")?, ".</w>");

        assert_eq!(49408, tokenizer.vocab.len());

        let vocab_start: Vec<String> = serde_json::from_str(
    ...
    #[test]
    fn test_encode_word() -> anyhow::Result<()> {
        let text = "dog.".to_string();
        let mut tokenizer = TOKENIZER.lock().unwrap_or_else(PoisonError::into_inner);
        let tokens = tokenizer.encode(&text)?;
        assert_eq!(tokens, vec![1929, 269]);
        Ok(())
    }

    #[test]
    fn test_encode_sentence() -> anyhow::Result<()> {
    ...
    fn test_tokenize() -> anyhow::Result<()> {
        let tokens = tokenize(&["a test picture".to_string(), "tokens".to_string()])?;
        let arr = tokens.to_array_view::<i64>()?;
        let arrays = arr
            .axis_iter(Axis(0))
    ...
    #[test]
    fn test_encode_decode_sentence() -> anyhow::Result<()> {
        let text = "The quick brown fox jumps over the lazy dog.".to_string();
        let mut tokenizer = TOKENIZER.lock().unwrap_or_else(PoisonError::into_inner);
        let tokens = tokenizer.encode(&text)?;
    ...
    #[test]
    fn test_encode_decode_multi_token_word() -> anyhow::Result<()> {
        let text = "thiswordhasmanytokens".to_string();
        let mut tokenizer = TOKENIZER.lock().unwrap_or_else(PoisonError::into_inner);
        let tokens = tokenizer.encode(&text)?;
        let output = tokenizer.decode(tokens);

        assert_eq!(output, "thiswordhasmanytokens ");
        Ok(())
    }
}
