use teloxide::{
    adaptors::{throttle::ThrottlingRequest, DefaultParseMode, Throttle},
...
pub trait BotExt {
    fn send_markdown<C, T>(
        ...
        chat_id: C,
        text: T,
    ) -> ThrottlingRequest<JsonRequest<SendMessage>>
    where
        C: Into<Recipient>,
        T: Into<Markdown>;
...
impl SendDocumentExt for ThrottlingRequest<MultipartRequest<SendDocument>> {
    fn markdown_caption<T>(self, value: T) -> Self
    where
        T: Into<Markdown>,
    {
